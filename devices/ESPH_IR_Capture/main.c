    ...
    for (;;) {
        ...
        if (sdk_system_get_time() - current_time > UINT16_MAX) {
            ...
            if (c > 0) {
                ...
                printf("ESPH RAW Format\n");
                for (i = 1; i < c; i++) {
                    char esph_code[] = "00";
                    
                    esph_code[0] = baseRaw_dic[(buffer[i] / IR_CODE_SCALE) / IR_CODE_LEN];
                    esph_code[1] = baseRaw_dic[(buffer[i] / IR_CODE_SCALE) % IR_CODE_LEN];
...
void user_init() {
    ...
    printf("\n\nESPH IR RAW Code Capture Tool v1.0\n");
    printf("by wwns\n\n");
